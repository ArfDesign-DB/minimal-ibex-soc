...
/**
 * Writes ASCII hex representation of number to output.
 *
 * @param h Number to output in hex
 */
void puthex(uint32_t h);
 ...
 * at the appropriate address given the `vector_num`.
 *
 * @param vector_num Which IRQ the handler is for, must be less than 32.
 *
 * @param handler_fn Function pointer to the handler function.
 *
 * @return 0 on success,
 *         1 if vector_num out of range,
 *         2 if handler is too far away.
 */
int install_exception_handler(uint32_t vector_num,
