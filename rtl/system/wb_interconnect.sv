...
module wb_interconnect #(
  ...
  always_comb begin
    // This block checks the wb_addr_i, and decides which device or
    // peripheral is matched with the wb_addr_i value.

    bootrom_sel = ((wb_adr_i & BOOTROM_MASK) == BOOTROM_BASE);

    sram_sel = (wb_adr_i >= SRAM_BASE) &&
               (wb_adr_i < (SRAM_BASE + SRAM_SIZE));
    ...
    pwm_sel = ((wb_adr_i & 32'hFFFF_FF00) == PWM_BASE);

    uart2_sel = ((wb_adr_i & 32'hFFFF_FF00) == UART2_BASE);
  end
  ...
  // which is matched to the wb_addr_i value.
  ...
  always_comb begin
    ...
    // Check which peripheral is matched, then route the addr, data, etc.
    // only to that peripheral.
