...
module ibex_demo_system #(
  ...
  parameter SRAMInitFile = "",  // .vmem image baked into the SRAM
                                // (passed down to wrapper_top/sram_model)
  parameter BootInitFile = "rtl/system/boot.mem",
  // boot ROM image; default = direct XIP boot (no SRAM dependency)
  parameter bit UseDffram = 1'b0,  // 1 = GF180 DFFRAM behavioral model
                                  // (ASIC netlist SRAM); 0 = sram_model
  // SPI clock divider for the XIP flash controller:
  // SCK = clk/(2*XipClkDiv).
  ...
  // =========================================================
  // SOC WRAPPER
  // (2:1 OBI arbiter → obi2wb → WB interconnect)
  // =========================================================
