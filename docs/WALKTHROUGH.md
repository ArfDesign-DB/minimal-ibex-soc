...
| flow `profile` | Sets which half of the flow this PC is for: `sim` (Verilator only), `fpga` (Vivado only), `full`, `auto` (default, inferred). The unused tool is reported `[SKIP]`, never `[FAIL]`, and never prompted for | GUI **Tool Profile**, `flows.ps1 profile <p>`, or `./ibex_soc.sh profile <p>` | Saved per-PC in `.toolpaths` / `.toolpaths.sh`. Use it when a machine will only ever do one half |
| flow `simregression` | The Verilator regression on Windows **without Vivado** — shells into MSYS2 UCRT64 and runs `./ibex_soc.sh regression` | GUI **Verilator Regression** | Prints MSYS2 install steps if it is not there. Same 11 sims, same PASS strings as xsim |
...
34d. **"It asks for my RISC-V GCC path on every run"** (field report, 2026-08-20) — the same exec-bit root cause wearing a different mask, and a lesson in swallowed errors. `setup` saved the path correctly, then ran `sw/freertos/build.sh --check-toolchain` to confirm it; on a tree without the exec bit that failed with *Permission denied*, which `2>/dev/null` hid, so the flow reported "RISC-V GCC not found" one line after saving a perfectly good path — and re-asked next run, since the probe failed identically every time. **Every `build.sh` invocation now goes through `bash build.sh`**, so the flow is immune to the exec bit regardless of how the tree was obtained (git, zip export, copy across a filesystem without permissions). If you add a script that invokes another script, invoke it with its interpreter.

34e. **`Illegal instruction ... at PC 0x20400000: 0xffffffff` from tb_freertos** — not a CPU or RTL bug: the **program image is missing**. `sw/freertos/build/freertos_demo_sim_flash.vmem` is built by the RISC-V toolchain, and without it the flash model returns all-ones, the CPU fetches `0xFFFFFFFF`, and the run becomes a multi-minute illegal-instruction spew (Verilator prints only a `%Warning: $readmem file not found` line, easily lost above the noise). `sim`/`regression` now check each bench's required image up front and either build it (toolchain present) or stop immediately naming what is missing and how to fix it. Any all-`0xFFFFFFFF` or all-`0x00000000` fetch loop in a new bench should send you to its image first.

34c. **`./ibex_soc.sh: Permission denied` on a fresh Linux clone** — fixed 2026-08-20, but know the shape of it: `ibex_soc.sh` and `sw/freertos/build.sh` were committed `100644` because they were created on Windows, where git does not track the executable bit (every *vendored* script is 755, which is what makes it easy to miss). The loud symptom is the documented command failing outright; the quiet one is worse — `ibex_soc.sh` invokes `build.sh` **directly**, so the toolchain probe failed on permissions and `setup` reported "RISC-V GCC not found" with a perfectly good toolchain installed. If you add a shell script on Windows, commit it with `git update-index --chmod=+x <file>` and verify with `git ls-files -s`.

34b. **Ubuntu 22.04: apt's Verilator is 4.210 and NO apt package fixes it** (`--timing`/`--binary`, which every testbench here needs, arrived in 5.0). Since 2026-08-20 `./ibex_soc.sh deps` **builds Verilator from source** into `~/ibex-tools/verilator` (pinned to the latest upstream release tag, ~5-10 min) — the Linux equivalent of the Windows GUI's automatic toolchain download — and `setup` offers to take a path to an existing build instead, remembering it in `.toolpaths.sh`. Tools are resolved by **version across every PATH entry**, not by PATH order, so a stale 4.x sitting first (the classic `export PATH=$HOME/verilator/bin:$PATH` case) can no longer mask a good 5.x further down. Sim flows refuse up front with the version they found rather than failing later inside a build log.
...
36. **`%Warning-WIDTHTRUNC` / `WIDTHEXPAND` from a simulation model — fatal in one flow, invisible in every other** (field report, 2026-08-20). `ibex_soc.sh` and `minimal_ibex_soc.core` both run Verilator with `-Wno-fatal`, so width warnings scroll past inside a build log; the upstream `lowrisc:ibex:demo_system` **sim target runs `-Wall` with warnings fatal**, so the identical file stops that build with `ERROR:`. Two models had the same real defect — a 24-bit address compared against an `int` byte count and then used as a memory index (`mem[a - BASE_OFFSET]` in `spi_nor_flash_model.sv`, `mem[addr_q % MEM_BYTES]` in `periph_models.sv`): three widths in one expression. Both fixed at the source (one 32-bit unsigned domain plus an explicitly sized index), not waived. What IS waived — inline, with the reason next to the code — is `BLKSEQ`: a bus-functional model must see its own updates within an edge, so its assignments are blocking by design. Two things worth knowing next time: `--top-module X` makes Verilator lint **only what X reaches**, so `lint` had never looked at the BFMs at all (they hang off testbenches, not off the SoC) — `do_lint` now lints all 7 model files as their own tops (~1 s, the upstream waiver set, failing with the file named); and Verilator 5.050 adds `PROCASSINIT` (a declaration initialiser plus a procedural assignment — move the init into an `initial`) which 5.020 does not have, so "lints clean here" is a statement about your version as much as your code.

37. **A hand-edit inside a generated netlist has no safety net** (found 2026-08-21, in the PD-handoff `ibex_soc_merged_blackbox_dffram.v`). The hand-added DFFRAM blackbox instance had `WE0/Di0/Do0` rotated — write-enable into the data port, the RAM's output shorted against the controller's wdata driver, rdata undriven: every SRAM read X, i.e. dead silicon. No flow here could catch it: the Verilator/xsim regression compiles the **RTL**, `tb_soc-dffram` proves the RTL's DFFRAM wiring, and the netlist sits in `verilog_out/` which nothing simulates. The two cheap checks that DO work, now the rule for any netlist hand-edit: (a) diff the instance port-by-port against the RTL generate it mirrors (`wrapper_top.sv` `gen_sram_dffram`), and (b) run `verilator --lint-only --top-module ibex_demo_system <netlist>` — the rotation showed up as MULTIDRIVEN/width warnings and 0-error elaboration confirms the fix. Sign-off simulation of the netlist itself stays PD's gate-level step.

38. **`reg x = <expression with signals>` is illegal Verilog — and only Icarus tells you** (field report, 2026-08-24). sv2v translates SystemVerilog's `logic unused_scramble_inputs = &{...signals};` (Ibex's lint-pacifier idiom) into a `reg` declaration whose initialiser references seven nets. A `reg` initialiser must be a **constant expression** in Verilog; Verilator and Yosys accept it silently, Icarus rejects it with seven `not allowed in a constant expression` errors on one line (netlist line 9405). Fix in a hand-maintained netlist: make it `wire x = <expr>;` — a net declaration with an initialiser is a continuous assignment, legal with any RHS, and behaviour-identical for an unused sink. Same pass silenced `.ram_cfg_i('b0)` (unsized literal → 32 bits against a 10-bit port; Icarus prunes with a warning) to `10'b0`. The netlist now compiles rc=0 under Icarus 13.0, elaborates 0-error under Verilator, and analyzes clean in xvlog — three engines, three different strictness profiles, and each has now caught something the other two passed.

39. **A parameter default in the handoff netlist IS the tapeout configuration** (found 2026-08-24). PD instantiates `ibex_demo_system` from merged.v in the Caravel wrapper; whatever they don't override, they get the default of. `GpiWidth` defaulted to **8** while the signed-off pad map bonds `gp_i[15:8]` as the camera bus CAM_D7..0 — a bare instantiation would silently drop the entire camera input path, and every tool compiles it happily (this repo never noticed because every FPGA top and testbench passes GpiWidth explicitly). Default changed to 16 in `ibex_demo_system.sv`, `wrapper_top.sv` and the netlist — behaviour-neutral here (nothing uses the default), correct-by-default for PD. The rule: before handoff, elaborate the netlist top **bare** and check every port width against the pad map — that is what the integrator gets.
