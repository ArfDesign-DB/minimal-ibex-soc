...
| Teammate / field robustness (2026-08-19 → 20) | Four field reports closed at the root: scripts committed non-executable (every script now invoked as `bash <script>`, plus `--chmod=+x`), "it asks for my GCC path every run" (a swallowed *Permission denied* from that same cause), `Illegal instruction 0xffffffff` from tb_freertos (a missing program image — benches now verify their image up front), and twelve `No such file or directory` from gcc (`build.sh` is now cwd-independent, both build scripts verify every source first, and `setup` on both OSes opens with a `git ls-files --deleted` integrity line). **Tool profiles** (`sim`/`fpga`/`full`/`auto`) let a PC run Verilator without Vivado or Vivado without Verilator — the unused tool is `[SKIP]`, never `[FAIL]`, never prompted for; Windows can now run the whole 11-sim Verilator regression through MSYS2 from the GUI. A fifth (2026-08-20) came from the other direction: the upstream FuseSoC sim target compiles the same simulation models with `-Wall` and fatal warnings, where a 24-bit-vs-`int` width mismatch in two of them stopped the build while every flow here stayed green - both fixed at the source, and `lint` now checks each of the 7 model files as its own top; re-verified after the change with Verilator 13/13 and an xsim re-run of the six benches that instantiate a changed model. A sixth (2026-08-24): Icarus on Shivanee's box rejected the handoff netlist — sv2v's `reg x = <signals>` is illegal Verilog that Verilator/Yosys accept silently; fixed as `wire` (plus `10'b0` on `ram_cfg_i`), and the same pass caught `GpiWidth` defaulting to 8 against the pad map's 16 — a bare Caravel instantiation would have silently dropped the camera bus. Netlist now rc=0 under Icarus 13.0, Verilator and xvlog |
...
1. **UART2 in the tapeout netlist — YES** (decided). UART1 = host/debug,
   ...
   *Second field report (2026-08-24):* her `iverilog` compile also fails —
   sv2v turned Ibex's `logic unused_scramble_inputs = &{...};` into a `reg`
   with a non-constant initialiser (illegal Verilog; only Icarus enforces
   it — 7 errors on netlist line 9405). Fixed as `wire`, plus
   `.ram_cfg_i(10'b0)` for the width warning. The same pass found
   **`GpiWidth` defaulting to 8** against the pad map's 16: a bare Caravel
   instantiation would silently drop the camera bus (`gp_i[15:8]` = CAM_D).
   Default is now 16 in RTL and netlist (behaviour-neutral here — every
   top/bench passes it explicitly; WALKTHROUGH gotchas 38-39). Netlist
   verified: **Icarus 13.0 rc=0, Verilator 0-error elaboration, xvlog
   clean**; repo lint gate green.
