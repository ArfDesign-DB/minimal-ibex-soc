...
// Clock half-period in nanoseconds: 25 ns at 20 MHz.

static constexpr double CLK_HALF_PERIOD_NS = 25.0;
 
// Clock half-period in picoseconds for Verilator time/trace: 25,000 ps.
...
<< std::endl;
  ...
  // Each time_ unit = one half-cycle = 25 ns = 25,000 ps at 20 MHz

  // So timestamp = time_ * 25,000 ps

  tracer_.dump(static_cast<vluint64_t>(GetTime()) * 25000UL);
